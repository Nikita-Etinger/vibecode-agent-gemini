...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "start_agent_loop_chatgpt" && info.selectionText) {
  ...
  try {
    ...
    if (result.success) {
      ...
      chrome.notifications.create({
        type: "basic",
        iconUrl: ICON_DATA,
        title: "Zero-Click (ChatGPT): Успех!",
      ...
      if (loopState.retries >= loopState.maxRetries) {
        ...
        chrome.notifications.create({
          type: "basic",
          iconUrl: ICON_DATA,
          title: "Zero-Click (ChatGPT): Превышен лимит",
      ...
      chrome.notifications.create({
        ...
        message: "Промпт с ошибкой автоматически отправляется в ChatGPT..."
      });
